...
void solution() {
  ...
  for (int i=0; i < m; ++i) {
    ...
    if (votes[a] > votes[winner] || (votes[a] == votes[winner] && a < winner)) {
      winner = a;
    }
  ...
  // should basically be just to keep count of current max.
  // So I should take out the max.
  // It is known which gets a vote since that is the input.
  // Therefore, I only need to check if the new vote is greater than the previous vote.
} 
