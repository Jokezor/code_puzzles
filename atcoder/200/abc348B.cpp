...
ll point_dist(pair<ll, ll> p1, pair<ll, ll> p2) {
  ll x_diff = p1.first - p2.first;
  ll y_diff = p1.second - p2.second;
...
void solution() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < n; ++j) {
      ...
      ll curr_dist = point_dist(points[i], points[j]);
      if (curr_dist > dist || (curr_dist == dist && farthest_point > j + 1)) {
        dist = curr_dist;
        farthest_point = j + 1;
      }
    }
