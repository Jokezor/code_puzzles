...
void solution() {
  ...
  //
  // Find smallest a+b+c+d+e such that the equation holds.
  //
  // 20 => d = 2
  //
  // e = n/15
  ...
  // So I want to have up to 30
  // Keep everything less than 30.
  while (n > 35) {
    ans++;
    n -= 15;
  }
  ...
  for (ll i = 0; i < coins.size(); ++i) {
    ...
    ll temp_ans = 0;
    ll j = i;
    while (temp_n && j < coins.size()) {
      temp_ans += temp_n / coins[j];
      temp_n %= coins[j];
      ++j;
    }

    xtra = min(xtra, temp_ans);
  }
  ans += xtra;
