...
void solution() {
  ...
  // Record for each index, the number of '<' and the number of '>' that it has prepending or trailing.
  // If at current index, both i+1, i-1 are different then take 0.
  // Otherwise take max(left, right)
  // Where left and right is the prefixsums
  //
  // Nope! its only the number of < or > in a row not total.

  int n = s.length();

  vector<ll> less_left_prefix_sum(n+2, 0);
  vector<ll> greater_left_prefix_sum(n+2, 0);

  for (int i=0; i < n; ++i) {
      if (s[i] == '<') {
          less_left_prefix_sum[i+1] = less_left_prefix_sum[i] + 1;
      }
      else if (s[i] == '>') {
          greater_left_prefix_sum[i+1] = greater_left_prefix_sum[i] + 1;
      }
  }

  // Now do from reverse
  vector<ll> less_right_prefix_sum(n+2, 0);
  vector<ll> greater_right_prefix_sum(n+2, 0);

  for (int i=n-1; i >= 0; --i) {
      if (s[i] == '<') {
          less_right_prefix_sum[i] = less_right_prefix_sum[i+1] + 1;
      }
      else if (s[i] == '>') {
          greater_right_prefix_sum[i] = greater_right_prefix_sum[i+1] + 1;
      }
  }
  ...
  for (int i=0; i <= n; ++i) {
      ...
      if (i == 0) {
          // Only look right
          num = greater_right_prefix_sum[i];
      }
      else if (i == n) {
          // Only look left
          num = less_left_prefix_sum[i];
      }
      else {
          // Special case, now we can either 1 or 0
          if (s[i-1] != s[i] && s[i-1] == '>') {
              num = 0;
          }
          // Normal case, now take longest sequence
          else {
              num = max(less_left_prefix_sum[i], greater_right_prefix_sum[i]);
          }
      }
      nums.push_back(num);
      ans += num;
