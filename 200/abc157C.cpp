...
int power_needed(int i, int k) {
    return ceil(log2(k) - log2(i));
}
...
void solution() {
  ...
  // smallest integer x >= 0
  // Sounds like it will be checking if any constraint disagrees.
  // Start with all -1s.
  // Then simply insert at positions.
  //
  // If a position has an integer != -1 and the new is not equal to existing
  // then we return -1.
  // Replace -1 with 1 on print.
