...
void solution() {
  ...
  // Simply take each k numbers from i=0 to n-k.
  // Compute (1+pi)/2 for all of them and keep max.
  int n, k;
  ...
  double ans = 0;
  double dice_sum = 0;
  // Instead we can do this rolling k at a time.
  for (int i=0; i < n; ++i) {
      dice_sum += (double)(1+p[i])/(double)2;

      if (i >= k) {
          dice_sum -= (double)(1+p[i-k])/(double)2;
      }
      ans = max(ans, dice_sum);
  }
  printf("%.13lf\n", ans);
