...
void solution() {
  ...
  // Seems its simply checking if all '#'
  // are connected horizontally or vetically.
  // Actually no.
  // Only requirement is that any '#' has at least one other '#'
  ...
  // Now go through each row, check if grid[i][j] == '#' and any adjacent is too.
  for (int i=0; i < h; ++i) {
      for (int j=0; j < w; ++j) {
          if (grid[i][j] == '#') {
              ...
              // Check right
              if (j+1 < w && grid[i][j+1] == '#') ++num_neighbours;

              // Check left
              if (j-1 >= 0 && grid[i][j-1] == '#') ++num_neighbours;

              // Check up
              if (i-1 >= 0 && grid[i-1][j] == '#') ++num_neighbours;

              // Check down
              if (i+1 < h && grid[i+1][j] == '#') ++num_neighbours;
