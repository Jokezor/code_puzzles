...
void solution() {
  ...
  // Manhattan distance, is it not?
  // If for each t, the manhattan distance needs to match the difference.
  //
  // distance = abs(x2-x1)+abs(y2-y1)
  // so t2 - t1 == distance then ok
  // else if t2 - t1 > distance, then we need to check that we can get back.
  // This means that ((t2-t1)-distance) % 2 ==0 so we can walk back and forth in place.
  int n;
  cin >> n;
  ...
  // Also need to check for first point.
  ...
  for (int i=0; i < n; ++i) {
      int distance = abs(x[i] - prev_x) + abs(y[i] - prev_y);
      int time_skip = t[i] - prev_time;
      if (time_skip < distance) {
          cout << "No\n";
          return;
      }
      // We won't have time to make it back since we need to jump back and forth.
      else if (time_skip > distance && (time_skip - distance) % 2 != 0) {
          // cout << "time_skip: " << time_skip << "dist: " << distance << "\n";
          cout << "No\n";
          return;
      }
      prev_x = x[i];
