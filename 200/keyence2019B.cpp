...
void solution() {
  ...
  for (int i=0; i < s.length(); ++i) {
      ...
      // Construct string and check if its keyence

      if (s[i] == 'k') {
          string word_found = "";
          word_found += s[i];
          // If k is not at 0, we cannot use skip!
          if (i > 0) {
              used_skip=true;
          }
          // Now find rest of letters
          ++current_char;
          found = true;

          // This only works for exact match?
          for (int j=i+1; j < s.length(); ++j) {
              if (s[j] == word_to_find[current_char]) {
                  word_found += s[j];
          ...
          // cout << word_found << "\n";
          // Check that if i not 0, we need to end on last char.
          if (i > 0 && last_ind < s.length()-1) {
              found = false;
          }
