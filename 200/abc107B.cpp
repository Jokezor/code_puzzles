...
void solution() {
  ...
  // The solution is simply to print only the columns and rows that do contains '#'.
  // Since they are unchanged by any operation.
  int h, w;
  cin >> h >> w;
  ...
  vector<bool> filled_rows(h, false);
  vector<bool> filled_columns(w, false);
  ...
  // Once we have the rows and columns to skip, we essentially
  // Check if the current row or column is in that set, if so we simply continue, else we print
  // the character at a[row][column]
  for (int i=0; i < h; ++i) {
      if (!filled_rows[i]) {
          continue;
      }
