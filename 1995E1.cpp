...
void solution() {
  ...
  ll ans = 0;
  // If n even, then we minimize the max of the pairs
  if (n % 2 == 0) {
    ll ma = 0;
    ll mi = MAX_VAL;

    for (int i = 0; i < n / 2; ++i) {
      ll s[4]{a[2 * i] + a[2 * i + 1], a[2 * i] + a[2 * i + n + 1],
              a[2 * i + n] + a[2 * i + 1], a[2 * i + n] + a[2 * i + n + 1]};
      sort(s, s + 4);

      // Do not understand why we take 3rd and second pos.
      // Take the 3rd as that is the smallest max.
      // And then what follows is that the 2nd must be the min, otherwise we
      // have done an illegal switch.
      ma = max(ma, s[2]);
      mi = min(mi, s[1]);
    }
    ans = ma - mi;
    ...
    // This I dont understand at all currently.
    vector<ll> r;
    ll curr = 0;

    for (int i = 0; i < n; ++i) {
      r.push_back(a[curr]);
      r.push_back(a[curr ^= 1]);
      curr = M(curr + n);
    }
    ...
    for (int id = 0; id < n; ++id) {
      for (int m1 = 0; m1 < 2; ++m1) {
        for (int m2 = 0; m2 < 2; ++m2) {
          ll mi_value = r[M(2 * id - m1)] + r[M(2 * id + 1 + m2)];
          vector<ll> dp[2]{vector<ll>(n, MAX_VAL), vector<ll>(n, MAX_VAL)};
          dp[m2][id] = mi_value;
          for (int j = 1; j < n; ++j) {
            int jd = (id + j) % n;
            int pd = (id + j - 1) % n;
            for (int pc = 0; pc < 2; ++pc) {
              for (int jc = 0; jc < 2; ++jc) {
                if (dp[pc][pd] != MAX_VAL &&
                    r[M(2 * jd - pc)] + r[M(2 * jd + 1 + jc)] >= mi_value) {
                  dp[jc][jd] = min(dp[jc][jd],
                                   max(dp[pc][pd], r[M(2 * jd - pc)] +
                                                       r[M(2 * jd + 1 + jc)]));
                }
              }
            }
