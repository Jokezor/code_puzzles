...
void solution() {
  ...
  // Simply xor all elements.
  // Then xor all elements if even, all except last if odd.
  //
  // Done if even.
  // If odd, then xor 2 last elements twice.
  ...
  if (n & 1) {
    // xor all except last
    pairs.push_back({1, n-1});

    // xor last 2, 2 times
    ans += 2;
    pairs.push_back({n-1, n});
    pairs.push_back({n-1, n});
  }
  else {
    pairs.push_back({1, n});
