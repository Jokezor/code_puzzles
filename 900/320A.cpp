...
void solution() {
  ...
  // Simply go through
  // each char, if the 3 chars 
  // 144, then we jump 3 chars to the right
  // If no match, check 2 chars, if they match 14, then we jump 2 chars to the right
  // If no match, then we check one char, if it match 1, then jump one char.
  // Else if no match, output NO.

  int i = 0;
  while (i < s.length()) {
    if (i < s.length() - 2) {
      string sub_match = s.substr(i, 3);
      if (sub_match == "144") {
        i += 3;
        continue;
      }
