...
void solution() {
  ...
  // Number of positions that can attack both queen and king.
  //
  // Stupid solution is to take each position from the queen and walk from there
  // Then check if that position can attack the king.
  // If so then add 1.
  ...
  // Simply check each of the 4 directions from the queen.
  // Add any that also attack the king we add.
  vector<pair<ll, ll>> positions = {{-a, -b}, {-a, b}, {a, -b}, {a, b}, {-b, -a}, {-b, a}, {b, -a}, {b, a}};
  set<pair<ll, ll>> queen_attacks;
  set<pair<ll, ll>> king_attacks;

  ll ans = 0;

  for (pair<ll, ll> position: positions) {
    pair<ll, ll> queen_attack_pos = {xq+position.first, yq + position.second};
    pair<ll, ll> king_attack_pos = {xk+position.first, yk + position.second};

    queen_attacks.insert(queen_attack_pos);
    king_attacks.insert(king_attack_pos);
  }

  for (pair<ll, ll> queen_attack : queen_attacks) {
    if (king_attacks.find(queen_attack) != king_attacks.end()) {
      ans++;
    }
  }
