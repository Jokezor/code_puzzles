...
void solution() {
  ...
  // First take unique for each.

  for (int i = 1; i <= k; ++i) {
    if (A_nums.find(i) != A_nums.end() && B_nums.find(i) == B_nums.end()) {
      A_count++;
      A_nums.erase(i);
    }
    if (B_nums.find(i) != B_nums.end() && A_nums.find(i) == A_nums.end()) {
      B_count++;
      B_nums.erase(i);
    }
  }
  ...
  for (int i = 1; i <= k; ++i) {
    if (A_nums.size() <= B_nums.size()) {
      if (A_count < k / 2 && A_nums.find(i) != A_nums.end()) {
        A_count++;
      } else if (B_count < k / 2 && B_nums.find(i) != B_nums.end()) {
        B_count++;
      }
    } else {
      if (B_count < k / 2 && B_nums.find(i) != B_nums.end()) {
        B_count++;
      } else if (A_count < k / 2 && A_nums.find(i) != A_nums.end()) {
        A_count++;
      }
    }
  }
