...
void solution() {
  ...
  // Solve it
    ...
    // Get highest bit of a and b.

    int highest_bit_a = 31;
    int highest_bit_b = 31;

    while (!(a & (1 << highest_bit_a))) --highest_bit_a;
    while (!(b & (1 << highest_bit_b))) --highest_bit_b;

    if (highest_bit_b > highest_bit_a) {
        cout << "-1\n";
        return;
    }
    if (a == b) {
    ...
    // Always do 2 operations.
    cout << "2\n";
    // Now make a into all 1s
    int new_a = (1 << (highest_bit_a+1)) -1;

    //cout << "highest bit a: " << highest_bit_a << "\n";
    //cout << "highest bit b: " << highest_bit_b << "\n";
    //cout << "new a: " << new_a << "\n";
