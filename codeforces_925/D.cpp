...
pair<ll, ll> factorize(ll n, ll x, ll y) {
  pair<ll, ll> factors;

  // n = k*x + l*y
  // n % x = (k*x)%x + (l*y)%x = (l*y)%x
  //
  // y%x

  ll l = 1, k = 0;
  ll l1 = 0, k1 = 1, x1 = x, y1 = y;

  while (y1) {
    ll q = x1 / y1;
    tie(l, l1) = make_tuple(l1, l - q * l1);
    tie(k, k1) = make_tuple(k1, k - q * k1);
    tie(x1, y1) = make_tuple(y1, x1 - q * y1);
  }
...
void solution() {
  ...
  // Now we can sort a and get it sorted while maintaining
  // indices.
  ll ans = 0;
  unordered_set<pair<ll, ll>, hash_pair> pairs;

  for (pair<ll, ll> a_i : a) {
    auto factors = factorize(a_i.first, x, y);

    ll target = 2 * factors.first * x - a_i.first;

    if (a.count(target) != 0) {
      ll target_ind = a[target];
      if (target_ind != a_i.second) {
        pair<ll, ll> index_pair = {target_ind, a_i.second};
        if (target_ind > a_i.second) {
          index_pair = {a_i.second, target_ind};
        }
        pairs.insert(index_pair);
      }
    }
  }
  ...
  // So now we can easily check in a if a number exists.
  //
  // m*x + n*y = a[i]
  // m*x - n*y = a[j]
  //
  // a[i] + a[j] = 2*m*x => a[j] = 2*m*x - a[i]
  // a[i] - a[j] = 2*n*y
  //
  // So we can simply look for a factorization of a[i] with x and y.
  //
  // So we can generate for all m*x + n*y the m*x-n*y in a unordered_map.

  // Only need to generate up to max(a).
}
