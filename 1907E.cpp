...
ll get_comb(tuple<ll, ll, ll> triple) {

  ll first = get<0>(triple);
  ll second = get<1>(triple);
  ll third = get<2>(triple);
...
ll get_digsum_triple(tuple<ll, ll, ll> triple) {
  ll digsum = 0;
  ...
  digsum += get_digsum(first);
  digsum += get_digsum(second);
  digsum += get_digsum(third);
...
void solution() {
  ...
  // First generate all 2nd pos
  // Then use those to generate 3rd pos

  set<tuple<ll, ll, ll>> triples;

  for (ll curr = n; curr >= n / 3; curr--) {
    if (curr % 10 <= n % 10) {
      triples.insert({curr, n - curr, 0});
    }
  }

  ll k = 1;
  for (auto good_triple : triples) {
    if (get<0>(good_triple) <= n - 2) {
      triples.insert({get<0>(good_triple), get<1>(good_triple) - 1, k});
      k++;
    }
  }
  ...
  for (auto good_triple : triples) {
    if (get_digsum_triple(good_triple) == n) {
      ans += get_comb(good_triple);
    }
  }
