...
void solution() {
  ...
  while (deals > k && prev_level < 35) {
      // If we have no more left, go to next level.
      if (!counts[prev_level]) {
          prev_level++;
      }

      // Move 3^k into the next power.
      ll bigger_purchase = 3;

      if (counts[prev_level] < bigger_purchase) {
          break;
      }
      counts[prev_level] = max(0ll, counts[prev_level] - bigger_purchase);
      counts[prev_level+1]++;

      ll prev_cost = pow(3, prev_level+1) + prev_level * pow(3, prev_level-1);
      ll next_cost = pow(3, prev_level+2) + (prev_level+1) * pow(3, prev_level);
      ans += next_cost - 3*prev_cost;
      deals -= 2;
  }
