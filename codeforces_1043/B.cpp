...
void solution() {
  ...
  // Simply find the biggest power of 10 less than n.
  // Then divide by (pow +1), if no remainder, then take the number.
  //

  ll n;
  cin >> n;

  ll pow_candidate = 1e18;
  vector<ll> result;

  while (pow_candidate >= 10) {
      ll possible_sol = n / (pow_candidate +1);
      if (possible_sol * (pow_candidate+1) == n) {
          result.push_back(n / (pow_candidate + 1));
      }
      pow_candidate /= 10;
  }
