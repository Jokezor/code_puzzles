...
void solution() {
  ...
  int n = s.length();
  // Go through each char and check how many common. O(N^2)
  int lower = -1;
  int higher = -1;
  ...
  while (i < n) {
      ...
      // Jump num_lower positions
      i += j;
      in_a_row = j;

      // Now we should check so we are not out of bonds.
      // Else we should check how many we have in a row.
      if (i > n-1) {
          break;
      }

      // Now go the second step, check that we have correct increment.
      j = 1;
      higher = s[i];
      while (i+j < n && s[i] == s[i+j]) {
          ++j;
      ...
      // We have found this many which match our condition of the same length.
      if (lower+1 == higher) {
          // We know there are min(in_a_row, j) matches.
          ans += min(in_a_row, j);
      }
