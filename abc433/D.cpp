...
unsigned long long concat_nums(string a, string b) {
    // They can be out of range!
    return stoull(a+b);
}
...
void solution() {
  ...
  ll ans = 0;
  // Go through each element and check if its concatenation is divisible by m.
  for (int i=0; i < n; ++i) {
      for (int j=0; j < n; ++j) {
          if (concat_nums(a[i], a[j]) % m == 0) {
              ++ans;
          }
      }
  }
